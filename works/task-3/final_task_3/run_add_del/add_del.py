...
class DeepStream_AddDel:
    ...
    def cb_newpad(self,decodebin,pad,data):
        ...
        if(gstname.find("video")!=-1):
            ...
            self.sinkpad = self.streammux.get_request_pad(pad_name)
            if pad.link(self.sinkpad) == Gst.PadLinkReturn.OK:
    ...
    def create_uridecode_bin(self,index,filename):
        ...
        bin.connect("pad-added",self.cb_newpad,self.g_source_id_list[index])
        bin.connect("child-added",self.decodebin_child_added,self.g_source_id_list[index])
    ...
    def stop_release_source(self,source_id):
        ...
        if state_return == Gst.StateChangeReturn.SUCCESS:
            ...
            self.sinkpad = self.streammux.get_static_pad(pad_name)
            #Send flush stop event to the sink pad, then release from the streammux
            self.sinkpad.send_event(Gst.Event.new_flush_stop(False))
            self.streammux.release_request_pad(self.sinkpad)
            print("STATE CHANGE SUCCESS\n")
            #Remove the source bin from the pipeline
            pipeline.remove(self.g_source_bin_list[source_id])
            source_id -= 1
            g_num_sources -= 1
    ...
    def delete_sources(self,data):
        ...
        for source_id in range(self.MAX_NUM_SOURCES):
            if (self.g_eos_list[source_id] and self.g_source_enabled[source_id]):
        ...
        source_id = random.randrange(0, self.MAX_NUM_SOURCES)
        while (not self.g_source_enabled[source_id]):
            source_id = random.randrange(0, self.MAX_NUM_SOURCES)
        #Disable the source
        self.g_source_enabled[source_id] = False
        #Release the source
    ...
    def add_sources(self,data):
        ...
        source_id = random.randrange(0, self.MAX_NUM_SOURCES)
        while (self.g_source_enabled[source_id]):
            source_id = random.randrange(0, self.MAX_NUM_SOURCES)
        ...
        # if (g_num_sources == self.MAX_NUM_SOURCES):
        #     GLib.timeout_add_seconds(10, self.delete_sources, self.g_source_bin_list)
        #     return False
    ...
    def main(self,args):
        ...
        for i in range(num_sources):
            ...
            if not source_bin:
                sys.stderr.write("Failed to create source bin. Exiting. \n")
                sys.exit(1)
            self.g_source_bin_list[i] = source_bin
            pipeline.add(source_bin)
        ...
        sink = Gst.ElementFactory.make(self.SINK_ELEMENT, "nvvideo-renderer")
        if not sink:
        ...
        pgie.set_property("gpu_id", self.GPU_ID)
        sgie1.set_property("gpu_id", self.GPU_ID)
        sgie2.set_property("gpu_id", self.GPU_ID)
        sgie3.set_property("gpu_id", self.GPU_ID)
        ...
        tiler.set_property("gpu_id", self.GPU_ID)
        nvvideoconvert.set_property("gpu_id", self.GPU_ID)
        nvosd.set_property("gpu_id", self.GPU_ID)

        #Set gpu ID of sink if not aarch64
        if(not is_aarch64()):
            sink.set_property("gpu_id", self.GPU_ID)
        ...
        #GLib.timeout_add_seconds(10, self.add_sources, g_source_bin_list)
...
if __name__ == '__main__':
    sys.exit(DeepStream_AddDel.main(sys.argv))
