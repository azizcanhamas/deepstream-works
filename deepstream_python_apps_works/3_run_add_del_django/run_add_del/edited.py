...
uri_list = ["file:///opt/nvidia/deepstream/deepstream-6.1/sources/deepstream_python_apps/apps/deepstream-preprocess-test/h264/v2.h264",
        "file:///opt/nvidia/deepstream/deepstream-6.1/sources/deepstream_python_apps/apps/deepstream-preprocess-test/h264/v2.h264",
        "file:///opt/nvidia/deepstream/deepstream-6.1/sources/deepstream_python_apps/apps/deepstream-preprocess-test/h264/v2.h264",
        "file:///opt/nvidia/deepstream/deepstream-6.1/sources/deepstream_python_apps/apps/deepstream-preprocess-test/h264/v2.h264"]
...
# Spesifik bir kaynagi ID ile silmek icin bu metot kullanilir.
def stop_release_source(source_id):
    ...
    if state_return == Gst.StateChangeReturn.SUCCESS:
        ...
        # Pipeline'dan ilgili kaynagi kaldir. (Parametre ornek: sink_0 "")
...
# Her calistiginda random olarak bir source'i siliyor.
# ONEMLI: Silme islemi stop_release_source metodunda gerceklesiyor.
#         Bu metot yalnizca silmeyi tetikliyor.
def delete_sources(data):
    ...
    # Veri akisinin bittigi kaynaklari sil.
    # Eger kaynak sona ulastiysa ve enable durumu True ise.
    ...
    #Eger kaynak sayisi 0 ise loop'dan cik.
    if (g_num_sources == 0):
        loop.quit()
    ...
    #Kaynagi tamamen durdur.
    print("Calling Stop %d " % source_id)
    stop_release_source(source_id)

    #Eger kaynak kalmadiysa loop'tan cik.
    if (g_num_sources == 0):
        loop.quit()
        print("All sources stopped quitting")
        return False
...
#Yeni bir kaynak icin bu metot kullanilir.
def add_sources(data):
    global g_num_sources
    global g_source_enabled
    global g_source_bin_list

    #Yeni kaynak icin ID atamasi
    source_id = g_num_sources

    #Yeni kaynagi aktif et (gorunur yap).
    g_source_enabled[source_id] = True

    print("Calling Start %d " % source_id)

    # Parametre olarak gelen URI adresini yeni kaynak icin decode et.
    source_bin = create_uridecode_bin(source_id, uri_list[0])
    
    #source_bin listesine kaynagi ekle ve pipeline'a ekle.
    ...
    #Eger state degistirme basariliysa kaynak ID'sini arttir.
    if state_return == Gst.StateChangeReturn.SUCCESS:
        print("STATE CHANGE SUCCESS\n")
        source_id += 1
    ...
    #Eger maksimum kaynak sayisina ulasildiysa, 10 saniyede bir delete_sources metodunu cagir.
    # NOT: loop delete_sources icinde kirilacaktir! delete_sources metodu bir kez tetiklenir.
...
def main(args):
    ...
    # Kaynak ekleme ve kaynak cikarma animasyonun baslatildigi satir.
    # 10 saniyede add_sources metodunun cagrilmasi planlanmis. Kaynak sinirina
    # ulasilirsa akis delete_sources'dan devam eder.
