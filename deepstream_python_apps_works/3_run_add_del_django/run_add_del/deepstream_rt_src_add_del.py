...
sys.path.append("/opt/nvidia/deepstream/deepstream/sources/deepstream_python_apps/apps/deepstream_django/rtspApp/run_add_del/")


class TrafficCamDeepstream:
    MAX_DISPLAY_LEN=64
    ...
    def stop_release_source(self,source_id):
        ...
        if state_return == Gst.StateChangeReturn.SUCCESS:
            ...
            self.pipeline.remove(g_source_bin_list[source_id])
            self.g_num_sources -= 1
    ...
    def delete_sources(self,data):
        ...
        for source_id in range(self.MAX_NUM_SOURCES):
            if (self.g_source_enabled[source_id]):
    ...
    def add_sources(self,data,uri):
        global g_num_sources
        global g_source_enabled
        global g_source_bin_list

        source_id = self.g_num_sources
    ...
    def main(self,args):
        ...
        self.g_num_sources = num_sources
        ...
        #sink = Gst.ElementFactory.make(SINK_ELEMENT, "nvvideo-renderer") ## RTSP SECTION -6
        ...
        streammux.set_property('width', self.MUXER_OUTPUT_WIDTH)
        streammux.set_property('height', self.MUXER_OUTPUT_HEIGHT)
        ...
        self.tiler.set_property("gpu_id", self.GPU_ID)
        ...
        sgie1.link(sgie2)
        sgie2.link(sgie3)
        sgie3.link(self.tiler)
        self.tiler.link(nvvideoconvert)
        ...
        #GLib.timeout_add_seconds(10, self.add_sources, self.g_source_bin_list)
...
if __name__ == '__main__':
    tcd=TrafficCamDeepstream()
    sys.exit(tcd.main(sys.argv))
